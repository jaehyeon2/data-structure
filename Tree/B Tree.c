...
typedef struct BTNode{
    int n;
    int isLeaf;         //leaf노드인 경우 1
    int isRoot;         //root노드인 경우 1
    int keys[M_WAY];    //3-way B-Tree이기 때문에 최대 2개의 키 값을 갖지만
...
BTNode* initBTNode(){       //leaf 노드 하나 생성 + 초기화
    int i;
    BTNode* newBTNode;
...
BTNode* BTInsert(BTNode* root, int key){
    ...
    if(root->isLeaf){
        ...
        //해당 노드가 root 노드인 경우레만 split 수행
        if(root->n==M_WAY&&root->isRoot){
            return splitChild(root);
        }
        //그냥 leaf노드인 경우 상위 함수에서 split 수행
        return root;
    }
    ...
    //해당 노드가 leaf노드가 아닐 때
        ...
        //해당 위치에 존재하는 child노드로 들어가 leaf노드까지 탐색
...
BTNode* splitChild(BTNode* root){
    ...
    root->keys[mid]=0;
    root->n--;
    //newParent 노드이 child 노드로 root, newSibling 노드를 연결
